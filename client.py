        ...
        while True:
            ...
            if message == "q":  # so the user can quit
                ...
                try:
                    ...
                    header = ("f" + str(len(file_contents)) +
                              "|" + filename + "|").encode()
                except:
...
def start_client(username, hostname, port):
    ...
    # first thing that is sent to the server is the username
    ...
    # main thread that constantly listens for messages from the server
    while client_run:
        try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "try again using the following format: python client.py [username] [hostname] [port]")
        sys.exit()
