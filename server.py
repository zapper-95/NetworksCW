...
# user input of 'q' will close the server socket
...
def quit_client(client_address, username):
    ...
    broadcast("{} has left the server".format(username),
              current_connections[client_address][1])
    del current_connections[client_address]

    # log for the server
    disconnect_by_message = "Disconnected by {} ({})".format(
...
def handle_client(client_socket, client_address, username):
    ...
    try:
        while True:
            # first byte used to determine if the client
            # wants to upload a file or send a message
            ...
            if header == "f":
                ...
                # now we know the size, we can iterate the correct number of times
                # to receive the entire file
                with open(username+"/"+file_name, "wb") as f:
                    for i in range(math.ceil(int(file_size)/1024)):
                        f.write(client_socket.recv(1024))

            elif header == "m":  # if the client wants to send a message
        ...
        # this would occur if a client leaves due to connection lost or q is pressed
        # on the client side
...
def start_server(port):
    ...
    # this is so the server can be closed with 'q'
    ...
    try:
        ...
        while True:
            ...
            # by doing this, I can ensure each upload folder is distinct
            ...
            # log for server
            connected_to_message = "Connected to by {} ({})".format(
                client_address, username)
            ...
            # when accepting new user, make a new thread to constantly handle their inputs
            client_thread = threading.Thread(target=handle_client, args=(
                client_socket, client_address, username))
            client_thread.daemon = True
            client_thread.start()

    except:
        # the server is closed when a error occurs in the main thread
